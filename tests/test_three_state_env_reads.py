...
TWO_STATE_READS_WITH_A_REASON: dict[str, str] = {
    ...
    "LOG_LEVEL": (
        "uvicorn's log verbosity. It names no host, origin, audience or credential and gates no "
        "route; an unrecognised value is uvicorn's own start-up error, not a relaxation."
    ),
    "NO_COLOR": (
        "scripts/guardrail_demo.py: the no-color.org convention, where the variable being "
        "PRESENT AT ALL (empty included) disables ANSI colour. Read with `is None`, so unset "
        "and emptied are already distinguished and the emptied case takes the more conservative "
        "branch."
    ),
    "PORT": (
