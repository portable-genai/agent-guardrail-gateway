...
The core is the code a client owns outright after an extraction: the policy, the decisions
and the port contracts. A cloud or model SDK import there is lock-in wearing a green gate.
The rule is an allowlist rather than an SDK blocklist, because a blocklist rots the day a
vendor renames a distribution.

THIS REPOSITORY HAS NO ``domain/`` PACKAGE. The gateway is a small stateless proxy, so its
core sits flat beside the wiring: ``models.py`` (the decision records), ``policy.py`` (the
bank-owned thresholds parsed out of ``config/settings.yaml``) and the ``ports/`` package (the
Protocols). :data:`CORE_LAYERS` therefore names modules as well as directories, and the scan
below accepts either. Everything else under ``src/guardrail_gateway`` is deliberately outside
that boundary and free to depend on its framework: ``config.py`` and ``schemas.py`` are the
YAML / pydantic edge, ``container.py`` is the wiring, and ``api/``, ``cli/`` and ``adapters/``
are adapters by definition.
...
CORE_LAYERS = ("models.py", "policy.py", "ports")
...
def test_the_scan_can_see_a_violation_in_a_flat_core_module(tmp_path: Path) -> None:
    ...
    module.write_text("from google.cloud import dlp\n", encoding="utf-8")
