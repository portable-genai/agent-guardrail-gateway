...
``tests/test_contract_parity.py`` rather than restated here. Both files must build the same
settings for the two proofs to be about the same thing, and a second copy of a bespoke
constructor is a copy that drifts. This repository keeps its suites flat (no
...
def main(profile: str) -> int:
    ...
    from tests.test_contract_parity import _load, _settings
