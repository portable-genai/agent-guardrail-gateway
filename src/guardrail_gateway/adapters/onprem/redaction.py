...
``onprem`` rebinds it here. The adapter constructs cleanly with **no external dependencies**
and structurally satisfies the same Protocol as the managed adapter, so the contract tests
prove interface parity. ``redact`` deliberately raises rather than passing text through
unredacted: an unimplemented redactor must never leak PII to a model or audit sink (P-04),
so porting on-premise *must* supply a real de-identifier. Filling this body in is the only
change required.
"""
